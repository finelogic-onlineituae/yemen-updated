...
<div>
    ...
    <div class="align-items-center text-center d-flex justify-content-center w-100 p-2 bg-form mh-100 h-100 ">
         <form action="@if(request()->has('edit')) {{ route('birth-certificate.store', ['application' => $application->id]) }} @else {{ route('application.confirm', ['application_id' => $application->id]) }}  @endif" enctype="multipart/form-data" method="POST" id="renew-passport-above-form" class="w-100 align-items-center text-center d-flex justify-content-center">
            @csrf
                ...
                <div class="card text-start my-2">
                    ...
                    <div class="card">
                            ...
                            <div class="card-body">
                                <div class="row">
                                <div class="form-group mb-3 col-lg-6 col-xl-6 col-md-6 col-sm-12">
                                        <label class="form-label fw-bold" for="profession">اسم الأب الرباعي</label>
                                    ...
                                    <div class="form-group mb-3 col-lg-6 col-xl-6 col-md-6 col-sm-12">
                                        <label class="form-label fw-bold" for="mother_nationality">جنسية الأب</label>
                               ...
                               <div class="row">
                                    <div class="form-group mb-3 col-lg-6 col-xl-6 col-md-6 col-sm-12">
                                        ...
                                        <input type="text" class="form-control" name="father_name" value="{{ $application->formable->father_name }}"  @if(!request('edit')) disabled @endif   required/>
                                        @error('father_name')<span class="text-danger">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="form-group mb-3 col-lg-6 col-xl-6 col-md-6 col-sm-12">
                                        <label class="form-label fw-bold" for="profession">جنسية الأم</label>
                                        <select class="form-select" name="father_nationality"  @if(!request('edit')) disabled @endif  required>
                                            <option value="">Choose a Country</option>
                                            @forelse ($countries as $country)
                                                <option value="{{ $country->id }}" @selected($country->id == $application->formable->father_nationality)>{{ $country->country_name }}</option>
                                            @empty
                ...
                <div class="card text-start my-2">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="form-group mb-3 col-lg-6 col-xl-6 col-md-6 col-sm-12">
                            <div @class(["d-none" => !request('edit'), "mb-2"])>
                                <label class="form-label fw-bold" for="father_passport_attachment"> جواز (ملف بي دي إف  ,jpg, png, jpeg)</label>
                                <input type="file" class="form-control" name="father_passport_attachment"  @if(!request('edit')) disabled @endif/>
                                 @error('father_passport_attachment') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                           <div>
                            <a onclick="openModal('father_passport_attachment')" class="btn btn-dark">View Father's Passport</a>
                            <!-- Modal -->
                                    <div id="father_passport_attachment" class="modal">
                                        <div class="modal-content">
                                            <span class="close" onclick="closeModal('father_passport_attachment')">&times;</span>
                                            @if(Str::of($application->formable->father_passport_attachment)->lower()->endsWith(['.jpg', '.jpeg', '.png', '.webp']))
                                                    <img src="{{ $application->formable->father_passport_attachment }}"
                                                        alt="Preview"
                                                        style="max-width: 100%;aspect-ratio: 1 / 1;object-fit: cover; max-height: 100%; object-fit: contain; border-radius: 6px;">
                                            @else
                                            <iframe id="passport_iframe" src="{{ $application->formable->father_passport_attachment }}"></iframe>
                                            @endif
                             ...
                             <div class="form-group mb-3 col-lg-6 col-xl-6 col-md-6 col-sm-12">
                                <div @class(["d-none" => !request('edit'), "mb-2"])>
                                <label class="form-label fw-bold" for="mother_passport_attachment"> الهوية الإماراتية (ملف بي دي إف  ,jpg, png, jpeg)</label>
                                <input type="file" class="form-control" name="mother_passport_attachment"  @if(!request('edit')) disabled @endif  />
                                @error('mother_passport_attachment')<span class="text-danger">{{ $message }}</span> @enderror
                                 </div>
                           <div>
                            <a onclick="openModal('mother_passport_attachment')" class="btn btn-dark">View Mother's Passport</a>
                            <!-- Modal -->
                                    <div id="mother_passport_attachment" class="modal">
                                        <div class="modal-content">
                                            <span class="close" onclick="closeModal('mother_passport_attachment')">&times;</span>
                                            @if(Str::of($application->formable->mother_passport_attachment)->lower()->endsWith(['.jpg', '.jpeg', '.png', '.webp']))
                                                    <img src="{{ $application->formable->mother_passport_attachment }}"
                                                        alt="Preview"
                                                        style="max-width: 100%;aspect-ratio: 1 / 1;object-fit: cover; max-height: 100%; object-fit: contain; border-radius: 6px;">
                                            @else
                                            <iframe id="emirate_id_iframe" src="{{ $application->formable->mother_passport_attachment }}"></iframe>
                                            @endif
            ...
            <div class="form-group my-3 text-center">
                @if(request()->has('edit'))
                    ...
                    <a class="btn btn-dark" id="submitBtn" href="{{ route('birth-certificate.verify', ['application_id' => $application->id]) }}">لا تغييرات</a>
                @else
                    <button class="btn buttom-effect" id="submitBtn" >تأكيد التطبيق</button>
                    <a class="btn btn-dark" id="submitBtn" href="{{ route('birth-certificate.verify', ['application_id' => $application->id, 'edit'=> true]) }}">قم بإجراء التغييرات</a>
                @endif
