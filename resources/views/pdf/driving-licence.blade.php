<x-certificate-print-layout :application="$application">
    <table style="text-align:left; border:1px solid" >
        ...
        <tr><th> رخصة القيادة الصادرة في</th><td>{{ $application->formable->drivingLicenceCenter->center_name }}</td></tr>
        <tr><th>رقم رخصة القيادة</th><td>{{ $application->formable->driving_licence_number }}</td></tr>
        <tr><th>تاريخ انتهاء صلاحية رخصة القيادة</th><td>{{ $application->formable->expire_on }}</td></tr>
        <tr><th>رخصة القيادة الصادرة في </th><td>{{ $application->formable->issued_on }}</td></tr>
        <tr><th>فئة المركبة </th><td>{{ $application->formable->vehicleCategory->category_name }}</td></tr>
