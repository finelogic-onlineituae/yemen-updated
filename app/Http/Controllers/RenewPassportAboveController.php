...
use App\Models\Form;
use App\Models\RenewPassportAbove;
use App\Models\Country;
use App\Models\PassportCenter;
use Storage;
class RenewPassportAboveController extends Controller
{
    public function createRenewPassportAbove(Request $request)
    {
        session([ 'category' => '3', 'app' => 'applications/renew-passport-above']);

        return view('renew-passport-above.create', ['countries' => Country::all(), 'passport_centers' => PassportCenter::all()]);
    }

    public function storeRenewPassportAbove(Request $request)
    {
        $request->validate([
     ...
     //dd($request->issued_by);
        ...
        if($request->has('application')){
         ...
         //   dd($application->formable->passport);
            ...
            $RenewPassportAbove = RenewPassportAbove::create([
            ...
            $application = auth()->user()->forms()->create([
                ...
                'formable_type' => \App\Models\RenewPassportAbove::class,
                'form_type_id' => '9'
            ]); 
        }
        
       
        return redirect()->route('renew-passport-above.verify', ['application_id' => $application->id]);
    }
    public function verifyRenewPassportAbove(Request $request)
    {
       ...
       // dd('test');
        ...
        return view('renew-passport-above.verify-renew-passport-above', ['application' => $application, 'countries' => Country::all(), 'passport_centers' =>  PassportCenter::all()]);
    }

    public function editRenewPassportAbove(Request $request)
    {
        $application_id = decrypt($request->application_id);
        $application = Form::findOrFail($application_id);
        session(['application_id' => $application_id]);
        session(['edit_application' => 'renew-passport-above']);
        
        return view('renew-passport-above.create');
    }
}
